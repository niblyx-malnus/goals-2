...
=>  |%
    ...
    --
=|  cards=(list card)
|_  [zones=(map zid zone) to-to-jumps=(map rid to-to-jump)]
+*  this   .
...
++  zn
  ...
  ::
  ++  update-rule
    |=  [=tid upds=(list tz-rule-update)]
    ^-  _this
    (do-updates upds:abet:(do-updates:(abed:rl tid) upds))
  ::
  ++  update-rule-args
    |=  [=tid =rid =args]
    ^-  _this
  ...
  ++  tz-to-utc-list
    ...
    ++  validate-time
      ...
      ^-  ?
      ?~  pof=(~(pof or order.zon) utc-time) :: previous offset
        |
      =(offset u.pof)
    --
  ::
