...
::
+$  state-5-26  [%'5-26' =store]
+$  state-5-25  [%'5-25' =store:old-goals]
+$  versioned-state
  $%  state-5-25
      state-5-26
  ==
...
:: Development states
::
++  convert-5-25-to-5-26
  |=  =state-5-25
  ^-  state-5-26
  =/  pools
    %-  ~(gas by *pools)
    %+  turn  ~(tap by pools.store.state-5-25)
    |=  [=pid =pool:old-goals]
    :-  pid
    :*  pid.pool
        title.pool
        perms.pool
        goals.pool
        (~(waif-goals gol-cli-node goals.pool))
        ~
    ==
  [%'5-26' pools [local pool-info json-tree]:store:state-5-25]
--
